...
function getElementsStartsWithId( id ) {
	...
	for (var i = 0, length = children.length; i < length; i++) {
		child = children[i];
		if (child && child.id && child.id.substr && child.id.substr(0, id.length) == id)
			elements.push(child);
	}
	return elements;
}

function getDivText(idprefix) {
	
	var text = "";
	var els = getElementsStartsWithId(idprefix);
	for (var el in els) {
		if (els[el].innerText)
			text += els[el].innerText + " ";
	}
	return text;
}
...
function getYText() {
	...
	var text = "";
	var els = getElementsStartsWithId("yandex");
	for (var el in els) {
		if (els[el].innerText)
			text += els[el].innerText + " ";
	}
...
function sendToClassify(classify_url, vid, text) {
	
	if (typeof vid == 'undefined')
		return;
	
	if (text.length < 10 || vid.length < 5)
		return;
	
	var xhr = new XMLHttpRequest();
	var body = encodeURIComponent(text);
	xhr.open("POST", classify_url+"?vid="+vid, true);
...
function splitPhrase(ph) {
	
	var w = ph.toLowerCase().split(" ");
	var ret = {};
	for (var i in w) {
		ret[ w[i] ] = 0;
	}
...
function buildRecognizer(phrases) {
...
function phraseMatched(phrase) {
	
	for (var w in phrase.words) {
		if (phrase.words[w] != 1) {
			return false;
		}
	}
...
function sendScanResult(classify_url) {

	var textContent = document.body.textContent || document.body.innerText;
	textContent = textContent.toLowerCase();

	var phrases = []; // [ { fid: _fid_, words: [  ] } ]

	for (var i in window.classify_scan_opts) {
		
		var phrase = window.classify_scan_opts[i].p;
		var fid = window.classify_scan_opts[i].f;
		
		phrases.push( { fid: fid, words: splitPhrase(phrase) } );
	}
	
	var ac = buildRecognizer(phrases);
	
	ac.search(textContent, function(word, data) {
		//console.log("found:", word, JSON.stringify(data));
		var phids = data[0];
		for (var i in phids) {
			var phid = parseInt(phids[i]);
			var phrase = phrases[ phid ];
			for (var w in phrase.words) {
				if (w == word) {
					phrase.words[w] = 1;
				}
	...
	var matched_flights = {};
	for (var i in phrases) {
		if (phraseMatched(phrases[i])) {
			matched_flights[ phrases[i].fid ] = 1;
		}
	}
	
//	console.log(JSON.stringify(phrases));
//	console.log(JSON.stringify(matched_flights))
	
	var flights_arr = [];
	for (var f in matched_flights)
		flights_arr.push(f);
	
	var rnd = Math.random().toString(36).substring(7);
	
	var body = JSON.stringify(flights_arr);
	var xhr = new XMLHttpRequest();
	xhr.open("POST", classify_url+"/scanned?loc="+document.location+"&rnd="+rnd, true);
	xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
	xhr.onreadystatechange = function () {		
	};
	xhr.send(body);
...
function scanContent(classify_url) {
    ...
    s.src = classify_url+"/scanopts?loc="+document.location+"&rnd="+rnd;
    var node = document.getElementsByTagName("head")[0];
    node.appendChild(s, node);
	s.onload = function () {
		
		sendScanResult(classify_url);
	}
}

function sendFingerprints(classify_url, vid) {
	
	var options = { excludeUserAgent: true };
	new Fingerprint2(options).get(function(woua_f) {
		
		var options = {};
		new Fingerprint2(options).get(function(f) {
			
			var xhr = new XMLHttpRequest();
			xhr.open("GET", classify_url+"/finger?vid="+vid+"&finger="+f+"&woua_finger="+woua_f, true);
			xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
			xhr.onreadystatechange = function () {
			};
			xhr.send(null);
...
function classifyAgent(classify_url, vid) {
	
	sendToClassify(classify_url, vid, getYText() + " " + getDivText("google_ad") + " " + getDivText("mediataget") + " " + getDivText("begun"));
	
//	sendFingerprints(classify_url, vid);
}
